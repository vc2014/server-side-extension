...
public class JavaPlugin extends qlik.sse.ConnectorGrpc.ConnectorImplBase {
    ...
    public void setMetadata(Metadata metadata) {
     ...
     public io.grpc.stub.StreamObserver<qlik.sse.ServerSideExtension.BundledRows> evaluateScript(
         ...
         if(header != null ) {
             if(header.getParamsCount() == 0) {
                 ...
                 if(result.getRowsCount() > 0) {
                     logger.info("row count is greater than zero");
             ...
             logger.info("The script request header is null");
         ...
         return new io.grpc.stub.StreamObserver<qlik.sse.ServerSideExtension.BundledRows>() {
             ...
             public void onNext(ServerSideExtension.BundledRows bundledRows) {
                 logger.info("onNext in evaluateScript called");
                 if(header != null) {
                     ...
                     if(result.getRowsCount() > 0) {
                         responseObserver.onNext(result);
                         logger.info("onNext in evaluateScript completed");
                     } else {
                         responseObserver.onError(new Throwable("An error occured in evalScript in evaluateScript."));
                     }
                 } else {
             ...
             @Override
             public void onCompleted() {
                 logger.info("onCompleted in evaluateScript called");
     ...
     private ServerSideExtension.BundledRows evalScript(ServerSideExtension.ScriptRequestHeader header, 
         ...
         logger.info("evalScript completed");
         return bundledRowsBuilder.build();
     ...
     private boolean evalScript(String script, ServerSideExtension.BundledRows.Builder bundledRowsBuilder, 
        ...
        //     bundledRows.rows.add(duals=self.get_duals(row, ret_type))
         for(int j = 0; j<1000;j++) {
             List<Dual> values = new ArrayList<Dual>();
             ServerSideExtension.Row.Builder rowBuilder = ServerSideExtension.Row.newBuilder();
             for (ServerSideExtension.DataType type : ServerSideExtension.DataType.values()) {
                 switch (type) {
             case STRING :
                 values.add(dualBuilder.setStrData("" + j).build());
                 break;
             case NUMERIC : 
                 values.add(dualBuilder.setNumData(Double.parseDouble(""+ j)).build());
                 break;
             case DUAL : 
                 values.add(dualBuilder.setStrData("" + j).setNumData(Double.parseDouble("" + j)).build());
                 break;
             ...
             rowBuilder.addAllDuals(values);
             bundledRowsBuilder.addRows(rowBuilder);
         }
